...
const isoDateSchema = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Use a YYYY-MM-DD date.').refine((value) => {
...
export const pilotCreateSchema = z.object({
  ...
  if (targetEnd && targetEnd < value.targetStartDate) {
    context.addIssue({ code: 'custom', path: ['targetEndDate'], message: 'End date must be on or after the start date.' })
  }
  value.checkpoints.forEach((checkpoint, index) => {
    if (checkpoint.dueDate < value.targetStartDate || (targetEnd && checkpoint.dueDate > targetEnd)) {
      context.addIssue({ code: 'custom', path: ['checkpoints', index, 'dueDate'], message: 'Checkpoint date must fall within the pilot date range.' })
    }
  })
})

export const pilotStatusUpdateSchema = z.object({
  status: z.enum(['active', 'paused', 'completed', 'cancelled']),
  outcome: z.enum(['continue', 'revise', 'stop']).optional(),
}).superRefine((value, context) => {
  if (value.status === 'completed' && !value.outcome) {
    context.addIssue({ code: 'custom', path: ['outcome'], message: 'Choose continue, revise, or stop when completing a pilot.' })
  }
  if (value.status !== 'completed' && value.outcome) {
    context.addIssue({ code: 'custom', path: ['outcome'], message: 'An outcome is only recorded when the pilot is completed.' })
  }
})
