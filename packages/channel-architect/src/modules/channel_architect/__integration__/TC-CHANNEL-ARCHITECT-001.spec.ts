...
test.describe("TC-CHANNEL-ARCHITECT-001: governed program and pilot workflow", () => {
  test("enforces role grants and keeps pilots bound to approved current versions", async ({
    page,
    browser,
  }) => {
    ...
    const createdResponse = await page.request.post("/api/channel_architect/programs", {
      data: createPayload,
    });
    ...
    const detailResponse = await page.request.get(
      `/api/channel_architect/programs?id=${created.programId}`,
    );
    ...
    const programListResponse = await page.request.get(
      "/api/channel_architect/programs?page=1&pageSize=25",
    );
    ...
    expect(programList.items.some((item: { id: string }) => item.id === created.programId)).toBe(
      true,
    );
    const filteredProgramsResponse = await page.request.get(
      `/api/channel_architect/programs?search=${encodeURIComponent(programName)}&status=draft`,
    );
    ...
    const previousOrganizationCookie = (await page.context().cookies()).find(
      (cookie) => cookie.name === "om_selected_org",
    );
    ...
    try {
      ...
      const reviewResponse = await reviewerPage.request.post(
        `/api/channel_architect/programs/${created.versionId}/review`,
        {
          data: {
            decision: "approved",
            rationale: "Acceptance test reviewer approved the current version.",
          },
        },
      );
      ...
      const approvedDetailResponse = await reviewerPage.request.get(
        `/api/channel_architect/programs?id=${created.programId}`,
      );
      ...
      const foreignDetailResponse = await reviewerPage.request.get(
        `/api/channel_architect/programs?id=${foreignProgram.programId}`,
      );
      ...
      expect(
        scopedList.items.some((item: { id: string }) => item.id === foreignProgram.programId),
      ).toBe(false);
      const foreignRevisionResponse = await reviewerPage.request.patch(
        "/api/channel_architect/programs",
        {
          data: {
    ...
    try {
      ...
      expect(list.items.some((program: { id: string }) => program.id === created.programId)).toBe(
        true,
      );
      const deniedCreateResponse = await employeePage.request.post(
        "/api/channel_architect/programs",
        {
    ...
    const pilotResponse = await page.request.post("/api/channel_architect/pilots", {
      data: pilotPayload,
    });
    ...
    try {
      ...
      const filteredPilotsResponse = await pilotViewerPage.request.get(
        `/api/channel_architect/pilots?search=${encodeURIComponent(pilotPayload.name)}&status=planned`,
      );
    ...
    const activateResponse = await pilotApi.patch(
      `/api/channel_architect/pilots/${pilot.pilotId}`,
      {
        data: { status: "active" },
      },
    );
    expect(activateResponse.status()).toBe(200);
    const prematureCompletionResponse = await pilotApi.patch(
      `/api/channel_architect/pilots/${pilot.pilotId}`,
      {
    ...
    const checkpointResponse = await pilotApi.patch(
      `/api/channel_architect/pilots/${pilot.pilotId}`,
      {
    ...
    const completeResponse = await pilotApi.patch(
      `/api/channel_architect/pilots/${pilot.pilotId}`,
      {
        data: { status: "completed", outcome: "continue" },
      },
    ...
    const reopenedPilotResponse = await pilotApi.patch(
      `/api/channel_architect/pilots/${pilot.pilotId}`,
      {
        data: { status: "active" },
      },
    ...
    const stalePilotResponse = await page.request.post("/api/channel_architect/pilots", {
      data: pilotPayload,
    });
    ...
    try {
      ...
      const rejectionResponse = await rejectionPage.request.post(
        `/api/channel_architect/programs/${revision.versionId}/review`,
        {
          data: {
            decision: "rejected",
            rationale: "Acceptance test verifies rejected versions cannot pilot.",
          },
        },
      );
    ...
    const archivedDetailResponse = await page.request.get(
      `/api/channel_architect/programs?id=${created.programId}`,
    );
    ...
    expect((await archivedPilotResponse.json()).error).toContain(
      "Archived programs cannot start new pilots",
    );
