...
function errorResponse(error: unknown, operation: string) {
  ...
  if (error instanceof z.ZodError)
    return NextResponse.json(
      { error: "Invalid program request.", issues: error.issues },
      { status: 400 },
    );
  ...
  try {
    ...
    if (id) {
      ...
      const versions = await em.find(
        ChannelArchitectProgramVersion,
        { programId: id, tenantId, organizationId, isActive: true, deletedAt: null },
        { orderBy: { versionNumber: "DESC" } },
      );
      const versionIds = versions.map((version) => version.id);
      const reviews = versionIds.length
        ? await em.find(ChannelArchitectProgramReview, {
    ...
    const search = z
      ...
      .max(120)
      .optional()
      .parse(params.get("search") || undefined);
    const status = z
    ...
    const [items, total] = await Promise.all([
      em.find(ChannelArchitectProgram, where, {
...
export async function POST(req: Request) {
  try {
    ...
    const { result } = await bus.execute<
      typeof input & { tenantId: string; organizationId: string },
      { programId: string; versionId: string; version: number }
    >("channel_architect.programs.create", {
      input: { ...input, tenantId: context.tenantId, organizationId: context.organizationId },
      ctx: context.ctx,
    });
  ...
  methods: {
    GET: {
      ...
      responses: [
        {
          status: 200,
          description: "Programs or a program with version history",
          schema: z.object({
      ...
      responses: [
        {
          ...
          schema: z.object({ programId: z.string(), versionId: z.string(), version: z.number() }),
        },
      ],
    ...
    PATCH: {
      ...
      responses: [
        {
          status: 200,
          description: "Version created or program archived",
      ...
      errors: [
        {
          status: 409,
          description: "Program version changed concurrently or program is already archived",
          schema: errorSchema,
        },
      ],
    },
