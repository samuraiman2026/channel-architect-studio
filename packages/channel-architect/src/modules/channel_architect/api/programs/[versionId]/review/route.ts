...
export async function POST(req: Request, { params }: { params: Promise<{ versionId: string }> }) {
  try {
    ...
    const { result } = await bus.execute<
      typeof input & { tenantId: string; organizationId: string; programVersionId: string },
      { reviewId: string }
    >("channel_architect.programs.review", {
      input: { ...input, programVersionId: versionId, tenantId: auth.tenantId, organizationId },
      ctx,
    });
    ...
    if (error instanceof z.ZodError)
      return NextResponse.json(
        { error: "Invalid review request.", issues: error.issues },
        { status: 400 },
      );
    if (error && typeof error === "object" && "code" in error && error.code === "23505") {
      return NextResponse.json(
        { error: "This version already has a final review decision." },
        { status: 409 },
      );
    }
  ...
  methods: {
    POST: {
      ...
      errors: [
        ...
        {
          status: 403,
          description: "Reviewer is not independent of the program version",
          schema: z.object({ error: z.string() }),
        },
        {
          status: 409,
          description: "A decision already exists",
          schema: z.object({ error: z.string() }),
        },
      ],
    },
