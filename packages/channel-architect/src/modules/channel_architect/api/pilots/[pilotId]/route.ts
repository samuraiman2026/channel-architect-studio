...
export async function PATCH(req: Request, { params }: { params: Promise<{ pilotId: string }> }) {
  try {
    ...
    const commandId = checkpointId ? 'channel_architect.pilots.update_checkpoint' : 'channel_architect.pilots.update_status'
    const { result } = checkpointId
      ? await bus.execute<{ tenantId: string; organizationId: string; pilotId: string; checkpointId: string; input: unknown }, { checkpointId: string; status: string }>(commandId, {
        input: { tenantId: auth.tenantId, organizationId, pilotId, checkpointId, input }, ctx,
      })
      : await bus.execute<{ tenantId: string; organizationId: string; pilotId: string; input: unknown }, { pilotId: string; status: string; outcome: string | null }>(commandId, {
        input: { tenantId: auth.tenantId, organizationId, pilotId, input }, ctx,
      })
    ...
    if (error instanceof z.ZodError) return NextResponse.json({ error: 'Invalid pilot update.', issues: error.issues }, { status: 400 })
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    PATCH: {
      ...
      requestBody: { contentType: 'application/json', schema: z.union([pilotStatusUpdateSchema, pilotCheckpointUpdateSchema.extend({ checkpointId: z.string().uuid() })]) },
      responses: [{ status: 200, description: 'Pilot state updated', schema: z.record(z.string(), z.unknown()) }],
      errors: [
        { status: 400, description: 'Invalid status transition', schema: z.object({ error: z.string() }) },
        { status: 409, description: 'Pilot or checkpoint changed concurrently', schema: z.object({ error: z.string() }) },
      ],
    },
  },
}
