...
function errorResponse(error: unknown, operation: string) {
  ...
  if (error instanceof z.ZodError)
    return NextResponse.json(
      { error: "Invalid pilot request.", issues: error.issues },
      { status: 400 },
    );
...
export async function GET(req: Request) {
  try {
    ...
    const [items, totalCount] = await Promise.all([
      em.find(ChannelArchitectPilot, where, {
    ...
    const checkpoints = pilotIds.length
      ? await em.find(
          ChannelArchitectPilotCheckpoint,
          { pilotId: { $in: pilotIds }, tenantId, organizationId, isActive: true, deletedAt: null },
...
export async function POST(req: Request) {
  try {
    ...
    const { result } = await bus.execute<
      ...
      { pilotId: string; checkpointIds: string[] }
  ...
  methods: {
    GET: {
      ...
      responses: [
        {
          status: 200,
          description: "Scoped pilot records",
          schema: z.object({
    ...
    POST: {
      ...
      responses: [
        {
          status: 201,
          description: "Pilot and checkpoints created",
          schema: z.object({ pilotId: z.string(), checkpointIds: z.array(z.string()) }),
        },
      ],
      errors: [
        {
          status: 409,
          description: "Program version is not the current approved version",
