...
export default function ChannelArchitectProgramsPage() {
  ...
  const selectedReview = detail?.reviews.find(
    (review) => review.programVersionId === selectedVersion?.id,
  );
  ...
  const canStartPilot = Boolean(
    selectedVersion &&
    selectedReview?.decision === "approved" &&
    selectedVersion.versionNumber === detail?.program.currentVersionNumber,
  );
  const economicTotal = Object.values(settings.economics).reduce(
  ...
  const firstProgramNumber =
    programTotalCount === 0 ? 0 : (programPage - 1) * PROGRAM_PAGE_SIZE + 1;
  ...
  function toggleArchetype(
    group: "primaryArchetypes" | "secondaryArchetypes",
    archetype: Archetype,
  ) {
    setSettings((current) => ({
      ...current,
  ...
  const refreshPrograms = React.useCallback(
    async (page = programPage) => {
      setLoading(true);
      const params = new URLSearchParams({
        page: String(page),
        pageSize: String(PROGRAM_PAGE_SIZE),
      });
  ...
  React.useEffect(() => {
    ...
    async function loadPermissions() {
      try {
        const response = await apiCall<{ granted?: string[]; userId?: string }>(
        ...
        if (cancelled) return;
        setGrantedFeatures(
          new Set(Array.isArray(response.result?.granted) ? response.result.granted : []),
        );
  ...
  const refreshPilots = React.useCallback(
    async (page = pilotPage) => {
  ...
  async function saveProgram() {
    ...
    const body =
      mode === "create"
        ...
        : {
            ...
            expectedVersion: detail?.program.currentVersionNumber,
            scenario,
            settings,
          };
    ...
    if (!response.ok || !response.result) {
      setNotice(
        messageFrom(
          response.result,
          "Could not save the program. Check your access and try again.",
        ),
      );
      setSaving(false);
      return;
    ...
    const targetProgramName = creating
      ? programName.trim()
      : (detail?.program.name ?? programName.trim());
  ...
  async function archiveProgram(program: Program) {
    if (
      !window.confirm(
        `Archive “${program.name}”? Its version and review history will remain available, but no new pilots can be started.`,
      )
    )
      return;
    const response = await apiCall(
      API,
      {
        ...
        body: JSON.stringify({
          ...
          expectedVersion: program.currentVersionNumber,
        }),
      },
      {},
    );
    if (!response.ok || !response.result) {
      setNotice(
        messageFrom(response.result, "Could not archive this program. Reload and try again."),
      );
      return;
    }
  ...
  async function decide(decision: "approved" | "rejected") {
    ...
    const response = await apiCall(
      `${API}/${encodeURIComponent(selectedVersion.id)}/review`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    if (!response.ok || !response.result) {
      setNotice(
        messageFrom(
          response.result,
          "Could not record the review. Confirm you have reviewer access.",
        ),
      );
      return;
    }
  ...
  async function createPilot() {
    if (
      ...
      checkpointDrafts.some((checkpoint) => !checkpoint.title.trim() || !checkpoint.dueDate)
    )
      return;
    setSavingPilot(true);
    const response = await apiCall(
      PILOT_API,
      {
        ...
        body: JSON.stringify({
          ...
          checkpoints: checkpointDrafts.map((checkpoint) => ({
    ...
    if (!response.ok || !response.result) {
      setNotice(
        messageFrom(
          response.result,
          "Could not start the pilot. Confirm the selected version is approved.",
        ),
      );
      return;
    }
    ...
    const alreadyUnfilteredFirstPage =
      pilotPage === 1 && !pilotSearch && pilotStatus === "all" && !pilotSearchInput.trim();
    setPilotSearchInput("");
    setPilotSearch("");
    setPilotStatus("all");
  ...
  async function updatePilot(
    ...
    outcome?: "continue" | "revise" | "stop",
  ) {
    const response = await apiCall(
      `${PILOT_API}/${encodeURIComponent(pilot.id)}`,
      {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        ...
        <div className="mx-auto max-w-6xl space-y-6 p-6">
          <header className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
                Draft, revise, and review partner-program designs. Generated guidance is a planning
                hypothesis, not a benchmark or commercial offer.
              </p>
            </div>
            {!mode && canManagePrograms && (
              <button
          ...
          {mode && (
            <section className="space-y-4 rounded-lg border bg-card p-5">
              <h2 className="text-lg font-medium">
                {mode === "create" ? "Create a partner program" : "Create a new version"}
              </h2>
              {mode === "create" && (
              ...
              <label className="block text-sm">
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    const seed =
                      SCENARIO_LIST.find((item) => item.id === event.target.value) ?? initialPreset;
                    setCompanyName(seed.label);
                    setArr(seed.arr);
                    setMotion(seed.motion);
                    setIcp(seed.icp);
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {(
                  [
                    ["Company / project", companyName, setCompanyName],
                    ["ARR context", arr, setArr],
                    ["Current sales motion", motion, setMotion],
                    ["Ideal customer", icp, setIcp],
                    ["Strategic partner goal", ask, setAsk],
                  ] as const
              ...
              <label className="block text-sm">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setSettings({ ...settings, stage: event.target.value as AxisSettings["stage"] })
                  }
                >
                  {[
                    "Pre-PMF",
              ...
              <section className="space-y-3 rounded-md border p-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Allocate 100% of planning attention. These are not commission rates.
                  </p>
                </div>
                  ...
                  {ECONOMIC_AXES.map(([key, label]) => (
                    <label className="block text-sm" key={key}>
                      ...
                      <input
                        ...
                        onChange={(event) => updateEconomicAxis(key, event.target.value)}
                      />
                    </label>
                  ))}
                </div>
                <p
                  className={`text-xs ${economicTotal === 100 ? "text-muted-foreground" : "text-destructive"}`}
                  role="status"
                >
                  Total: {economicTotal}%{economicTotal !== 100 ? " (must equal 100%)" : ""}
                </p>
              </section>
              {(["primaryArchetypes", "secondaryArchetypes"] as const).map((group) => (
                <fieldset className="space-y-2" key={group}>
                  <legend className="text-sm font-medium">
                    {group === "primaryArchetypes"
                      ? "Primary partner types"
                      : "Secondary partner types"}
                  </legend>
                  <p className="text-xs text-muted-foreground">
                    {group === "primaryArchetypes"
                      ? "Choose at least one type to anchor the design."
                      : "Optional supporting partner types."}
                  </p>
                    ...
                    {ARCHETYPES.map((archetype) => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded border px-3 py-1.5 text-sm ${active ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:border-primary"}`}
                        >
                          {archetype}
                        </button>
                      );
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={
                    saving || !companyName.trim() || !icp.trim() || !ask.trim() || !canSaveDesign
                  }
                >
                  {saving ? "Saving…" : mode === "create" ? "Create program" : "Save new version"}
                </button>
                <button
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(240px,0.8fr)_minmax(0,1.6fr)]">
            <section className="rounded-lg border bg-card p-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">{programTotalCount} total</span>
              </div>
              <div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                <input
                  aria-label="Search programs"
                  placeholder="Search program names"
                  className="min-w-0 rounded border bg-background px-3 py-2 text-sm"
                  value={programSearchInput}
                  onChange={(event) => setProgramSearchInput(event.target.value)}
                  maxLength={120}
                />
                <select
                  ...
                  value={programStatus}
                  onChange={(event) => {
                    setProgramStatus(event.target.value as typeof programStatus);
                    setProgramPage(1);
                  }}
                >
              ...
              {loading ? (
                ...
                <p className="text-sm text-muted-foreground">
                  {programSearch || programStatus !== "all"
                    ? "No programs match these filters."
                    : "No programs yet. Create one to start a shared, versioned design."}
                </p>
              ) : (
                <ul className="space-y-2">
                  {programs.map((program) => (
                    <li key={program.id}>
                      <button
                        className={`w-full rounded border px-3 py-2 text-left ${detail?.program.id === program.id ? "border-primary bg-muted" : ""}`}
                        onClick={() => void openProgram(program)}
                      >
                        <span className="block font-medium">{program.name}</span>
                        <span className="text-xs text-muted-foreground">
                          v{program.currentVersionNumber} · {program.status}
                        </span>
                      </button>
                    </li>
                  ))}
              ...
              <div className="mt-3 flex items-center justify-between gap-2 border-t pt-3 text-xs">
                <span className="text-muted-foreground">
                  Showing {firstProgramNumber}–{lastProgramNumber}
                </span>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={() => setProgramPage((page) => Math.max(1, page - 1))}
                  >
                    Previous
                  </button>
                  ...
                  <button
                    ...
                    disabled={programPage >= programPageCount}
                    onClick={() => setProgramPage((page) => Math.min(programPageCount, page + 1))}
                  >
                    Next
                  </button>
                </div>
            ...
            <section className="min-w-0 rounded-lg border bg-card p-4">
              {!detail || !selectedVersion ? (
                <p className="text-sm text-muted-foreground">
                  Select a program to inspect versions and review history.
                </p>
              ) : (
                <div className="space-y-5">
                  <header className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <h2 className="text-xl font-semibold">{detail.program.name}</h2>
                      <p className="text-xs text-muted-foreground">
                        Owner user ID: {detail.program.ownerUserId}
                      </p>
                    </div>
                    {detail.program.status !== "archived" && canManagePrograms ? (
                      <div className="flex gap-2">
                        <button
                        ...
                        <button
                          className="rounded-md border px-3 py-2 text-sm"
                          onClick={() => void archiveProgram(detail.program)}
                        >
                          Archive program
                        </button>
                      </div>
                    ) : detail.program.status === "archived" ? (
                      <span className="rounded border px-3 py-2 text-sm text-muted-foreground">
                        Archived
                      </span>
                    ) : null}
                  </header>
                  <div className="flex flex-wrap gap-2">
                    {detail.versions.map((version) => {
                      const review = detail.reviews.find(
                        (item) => item.programVersionId === version.id,
                      );
                      const stateLabel = getProgramVersionStateLabel(
                        version.versionNumber,
                        detail.program.currentVersionNumber,
                        review?.decision,
                      );
                      return (
                        <button
                          key={version.id}
                          className={`rounded border px-3 py-1.5 text-sm ${selectedVersion.id === version.id ? "border-primary bg-muted" : ""}`}
                  ...
                  <div className="rounded-md border p-3 text-sm">
                    <p>
                      <strong>{selectedVersion.scenarioSnapshot.label}</strong> · engine{" "}
                      {selectedVersion.engineVersion}
                    </p>
                    <p className="mt-1 text-muted-foreground">
                      Goal: {selectedVersion.scenarioSnapshot.ask}
                    </p>
                    <p className="mt-2">
                      Planning emphasis:{" "}
                      {ECONOMIC_AXES.map(
                        ([key, label]) =>
                          `${label} ${selectedVersion.settingsSnapshot.economics[key]}%`,
                      ).join(" · ")}
                    </p>
                    <p className="mt-1">
                      Primary: {selectedVersion.settingsSnapshot.primaryArchetypes.join(", ")}
                    </p>
                    <p className="mt-1">
                      Secondary:{" "}
                      {selectedVersion.settingsSnapshot.secondaryArchetypes.join(", ") || "None"}
                    </p>
                    <p className="mt-1">Stage: {selectedVersion.settingsSnapshot.stage}</p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Saved {new Date(selectedVersion.createdAt).toLocaleString()} · creator{" "}
                  ...
                  {selectedReview ? (
                    <div className="rounded-md border p-3 text-sm">
                      <p className="font-medium">
                        {selectedReview.decision} by {selectedReview.reviewerUserId}
                      </p>
                      <p className="mt-1">{selectedReview.rationale}</p>
                    </div>
                  ) : canReviewSelectedVersion ? (
                    <div className="space-y-3 rounded-md border p-3">
                      <label className="block text-sm">
                        Review rationale
                        <textarea
                          className="mt-1 min-h-24 w-full rounded border bg-background px-3 py-2"
                          value={rationale}
                      ...
                      <p className="text-xs text-muted-foreground">
                        The API requires reviewer permission. A decision applies only to this
                        version.
                      </p>
                    </div>
                  ) : (
                    <p className="rounded-md border p-3 text-sm text-muted-foreground">
                      {isReviewAuthor
                        ? "The program owner and version creator cannot review this version."
                        : "This version has no review decision. Reviewer access is required to record one."}
                    </p>
                  )}
                  {canStartPilot && canManagePilots && (
                    <section className="space-y-3 rounded-md border p-4">
                      <div>
                        <h3 className="font-medium">Start a pilot from this approved version</h3>
                        <p className="mt-1 text-xs text-muted-foreground">
                          The pilot keeps this exact version as its governing program. Do not enter
                          individual partner or customer details in the cohort label.
                        </p>
                      </div>
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        <label className="block text-sm">
                          ...
                          <input
                            ...
                            className="mt-1 w-full rounded border bg-background px-3 py-2"
                            value={pilotEndDate}
                            onChange={(event) => setPilotEndDate(event.target.value)}
                          />
                        </label>
                      </div>
                      <fieldset className="space-y-3">
                        <legend className="text-sm font-medium">Pilot checkpoints</legend>
                        {checkpointDrafts.map((checkpoint, index) => (
                          <div
                            className="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(180px,0.5fr)_auto]"
                            key={index}
                          >
                            <label className="block text-sm">
                              Checkpoint {index + 1}
                        ...
                        <button
                          ...
                          disabled={checkpointDrafts.length >= 30}
                          onClick={() =>
                            setCheckpointDrafts((current) => [
                              ...current,
                              { title: "", dueDate: "" },
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          checkpointDrafts.some(
                            (checkpoint) => !checkpoint.title.trim() || !checkpoint.dueDate,
                          )
                        }
                        onClick={() => void createPilot()}
                      >
                        {savingPilot ? "Saving…" : "Create pilot"}
                      </button>
                    </section>
                  )}
                  {canStartPilot && permissionsReady && !canManagePilots && (
                    <p className="rounded-md border p-3 text-sm text-muted-foreground">
                      This version is approved and current. Pilot-management access is required to
                      start a pilot.
                    </p>
                  )}
                  <details>
                    ...
                    <div className="mt-3 space-y-4">
                      {Object.entries(selectedVersion.outputSnapshot).map(([key, value]) => (
                        <section key={key} className="rounded border p-3">
          ...
          <section className="space-y-3 rounded-lg border bg-card p-4">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="font-medium">Pilots</h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  Each pilot is tied to an approved program version. Completion requires a continue,
                  revise, or stop decision.
                </p>
                {permissionsReady && !canManagePilots && (
                  <p className="mt-1 text-xs text-muted-foreground">
                    View only. Pilot-management access is required to create pilots, update
                    statuses, or resolve checkpoints.
                  </p>
                )}
              </div>
              <div className="flex items-center gap-3 text-sm">
                <span className="text-muted-foreground">
                  Showing {firstPilotNumber}–{lastPilotNumber} of {pilotTotalCount}
                </span>
                <button
                  type="button"
                  className="rounded border px-2 py-1 disabled:opacity-50"
                  aria-label="Previous pilots page"
                  disabled={pilotPage <= 1}
                  onClick={() => setPilotPage((page) => Math.max(1, page - 1))}
                >
                  Previous
                </button>
                <span aria-live="polite">
                  Page {pilotPage} of {pilotPageCount}
                </span>
                <button
                  type="button"
                  className="rounded border px-2 py-1 disabled:opacity-50"
                  aria-label="Next pilots page"
                  disabled={pilotPage >= pilotPageCount}
                  onClick={() => setPilotPage((page) => Math.min(pilotPageCount, page + 1))}
                >
                  Next
                </button>
              </div>
            ...
            <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
              <input
                ...
                placeholder="Search pilot names"
                className="min-w-0 rounded border bg-background px-3 py-2 text-sm"
                value={pilotSearchInput}
                onChange={(event) => setPilotSearchInput(event.target.value)}
                maxLength={120}
              />
              <select
                aria-label="Filter pilots by status"
                className="rounded border bg-background px-3 py-2 text-sm"
                value={pilotStatus}
                onChange={(event) => {
                  setPilotStatus(event.target.value as typeof pilotStatus);
                  setPilotPage(1);
                }}
              >
                <option value="all">All statuses</option>
            ...
            {pilots.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {pilotSearch || pilotStatus !== "all"
                  ? "No pilots match these filters."
                  : "No pilots yet. Approve the current version of a program to start one."}
              </p>
            ) : (
              <ul className="space-y-3">
                {pilots.map((pilot) => (
                  <li key={pilot.id} className="rounded-md border p-3">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h3 className="font-medium">{pilot.name}</h3>
                        <p className="text-sm text-muted-foreground">
                          {pilot.cohortLabel} · owner {pilot.ownerUserId} ·{" "}
                          {pilot.targetStartDate.slice(0, 10)}
                          {pilot.targetEndDate ? ` to ${pilot.targetEndDate.slice(0, 10)}` : ""}
                        </p>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {pilot.programName} · version{" "}
                          {pilot.programVersionNumber ?? "unavailable"} · {pilot.status}
                          {pilot.outcome ? ` · outcome: ${pilot.outcome}` : ""}
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {canManagePilots && (
                          <>
                            {pilot.status === "planned" && (
                              <>
                                <button
                                  ...
                                  onClick={() => void updatePilot(pilot, "active")}
                                >
                                  Start
                                </button>
                                <button
                                  className="rounded border px-3 py-1.5 text-sm"
                                  onClick={() => void updatePilot(pilot, "cancelled")}
                                >
                                  Cancel
                                </button>
                              </>
                            ...
                            {pilot.status === "active" && (
                              <>
                                <button
                                  className="rounded border px-3 py-1.5 text-sm"
                                  onClick={() => void updatePilot(pilot, "paused")}
                                >
                                  Pause
                                </button>
                                <button
                                ...
                                {(["continue", "revise", "stop"] as const).map((outcome) => (
                                  <button
                                    key={outcome}
                                    className="rounded border px-3 py-1.5 text-sm disabled:opacity-50"
                                    disabled={pilot.checkpoints.some(
                                      (checkpoint) => checkpoint.status === "planned",
                                    )}
                                    onClick={() => void updatePilot(pilot, "completed", outcome)}
                                  >
                                    Complete: {outcome}
                                  </button>
                                ))}
                    ...
                    {pilot.status === "active" &&
                      pilot.checkpoints.some((checkpoint) => checkpoint.status === "planned") && (
                        <p className="mt-2 text-xs text-muted-foreground">
                          Complete or skip all checkpoints before recording an outcome.
                        </p>
                      )}
                    <ul className="mt-3 space-y-2">
                      {pilot.checkpoints.map((checkpoint) => (
                        ...
                        >
                          <span>
                            {checkpoint.title} · due {checkpoint.dueDate.slice(0, 10)} ·{" "}
                            {checkpoint.status}
                          </span>
                          {canManagePilots &&
                            ...
                            (pilot.status === "active" || pilot.status === "paused") && (
                              <span className="flex gap-2">
                                <button
                                  ...
                                  onClick={() =>
                                    void updateCheckpoint(pilot, checkpoint, "completed")
                                  }
                                >
                                  Complete
                                </button>
                                <button
                                  className="rounded border px-2 py-1 text-xs"
                                  onClick={() =>
                                    void updateCheckpoint(pilot, checkpoint, "skipped")
                                  }
                                >
                                  Skip
                                </button>
