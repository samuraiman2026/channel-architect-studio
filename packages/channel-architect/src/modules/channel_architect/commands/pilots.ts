...
const createPilot: CommandHandler<CreatePilotInput, { pilotId: string; checkpointIds: string[] }> =
  {
    ...
    async execute(raw, ctx) {
      ...
      if (!actorId)
        throw new CrudHttpError(401, { error: "Authenticated pilot owner is required." });
      ...
      if (program.status === "archived")
        throw new CrudHttpError(409, { error: "Archived programs cannot start new pilots." });
      if (!review || review.decision !== "approved") {
        throw new CrudHttpError(409, {
          error: "A pilot can only start from an approved program version.",
        });
      }
      if (program.currentVersionNumber !== version.versionNumber) {
        throw new CrudHttpError(409, {
          error: "Approve the current program version before starting a pilot.",
        });
      }
      ...
      const pilot = em.create(ChannelArchitectPilot, {
        ...
        targetEndDate: parsed.targetEndDate
          ? new Date(`${parsed.targetEndDate}T00:00:00.000Z`)
          : null,
        outcome: null,
        createdBy: actorId,
      });
      const checkpoints = parsed.checkpoints.map((checkpoint, index) =>
        em.create(ChannelArchitectPilotCheckpoint, {
          id: randomUUID(),
          tenantId: raw.tenantId,
          organizationId: raw.organizationId,
          pilotId,
      ...
      await withAtomicFlush(
        ...
        [
          async () => {
            const currentProgram = await em.findOne(
              ChannelArchitectProgram,
              {
                id: program.id,
                tenantId: raw.tenantId,
            ...
            if (currentProgram.status === "archived")
              throw new CrudHttpError(409, {
                error: "The program was archived while the pilot was being created.",
              });
            if (currentProgram.currentVersionNumber !== version.versionNumber) {
              throw new CrudHttpError(409, {
                error:
                  "The program changed while the pilot was being created. Approve its current version and try again.",
              });
            }
...
const updatePilotStatus: CommandHandler<
  UpdatePilotInput,
...
> = {
  ...
  async execute(raw, ctx) {
    ...
    ensureOrganizationScope(ctx, raw.organizationId);
    if (!ctx.auth?.sub)
      throw new CrudHttpError(401, { error: "Authenticated actor is required to update a pilot." });
    const em = (ctx.container.resolve("em") as EntityManager).fork();
    ...
    if (!isPilotTransitionAllowed(pilot.status, parsed.status as PilotStatus)) {
      throw new CrudHttpError(409, {
        error: `A pilot cannot move from ${pilot.status} to ${parsed.status}.`,
      });
    }
    if (parsed.status === "completed") {
      const checkpointStatuses = await em.find(
        ChannelArchitectPilotCheckpoint,
        {
          pilotId: pilot.id,
          tenantId: raw.tenantId,
      ...
      if (!canCompletePilot(checkpointStatuses.map((checkpoint) => checkpoint.status))) {
        throw new CrudHttpError(409, {
          error: "Complete or skip every checkpoint before recording the pilot outcome.",
        });
      }
    }
    ...
    if (updated !== 1)
      throw new CrudHttpError(409, {
        error: "Pilot changed. Reload it before updating its status.",
      });
...
const updatePilotCheckpoint: CommandHandler<
  ...
  async execute(raw, ctx) {
    ...
    if (!ctx.auth?.sub)
      throw new CrudHttpError(401, {
        error: "Authenticated actor is required to update a checkpoint.",
      });
    const em = (ctx.container.resolve("em") as EntityManager).fork();
    await withAtomicFlush(
      em,
      [
        async () => {
          ...
          if (!canUpdatePilotCheckpoints(pilot.status)) {
            throw new CrudHttpError(409, {
              error: "Checkpoints can only be updated while a pilot is active or paused.",
            });
          }
          const checkpoint = await em.findOne(
            ChannelArchitectPilotCheckpoint,
            {
              id: raw.checkpointId,
              pilotId: pilot.id,
          ...
          if (updated !== 1)
            throw new CrudHttpError(409, {
              error: "Checkpoint changed. Reload the pilot before updating it.",
            });
        },
      ],
      { transaction: true },
