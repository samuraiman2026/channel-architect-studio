...
The 0.6.x generator discovered the package's entities, API routes, and backend page in a temporary sandbox. A standalone-layout check also confirmed package resolution from `node_modules` and runtime loading of all three entity classes. The build preserves legacy decorators because MikroORM's legacy decorators are not compatible with the standard decorator transform. A clean full host typecheck is still blocked by unrelated sandbox errors, and OpenAPI generation hits a missing `isolated-vm` native build under Node 26.7. Migration generation requires a reachable PostgreSQL database and remains unconfirmed; no migrations have been applied. End-to-end authorization checks remain outstanding. Do not apply migrations to a live environment as part of package setup.
