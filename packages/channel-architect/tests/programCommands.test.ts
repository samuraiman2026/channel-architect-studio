...
import {
  ...
  reviseProgram,
  reviewProgram,
} from "../src/modules/channel_architect/commands/programs";
import {
  createPilot,
  updatePilotCheckpoint,
  updatePilotStatus,
} from "../src/modules/channel_architect/commands/pilots";
import {
  ...
  const em = {
    ...
    persist(value: unknown) {
      ...
      for (const row of rows) {
        ...
        if (row.entity === ChannelArchitectProgramReview)
          reviews.push(row as Record<string, unknown>);
        if (row.entity === ChannelArchitectProgramVersion)
          options.persistedVersions?.push(row as Record<string, unknown>);
      }
    },
  };
  return {
...
function pilotCommandContext(
  ...
  const program = {
    ...
    currentVersionNumber: options.currentVersionNumber ?? 1,
    tenantId: "tenant-1",
    organizationId: "org-1",
    isActive: true,
    deletedAt: null,
  ...
  const version = {
    id: "a5892d87-b14c-4595-9e69-2b289c0cc610",
  ...
  const em = {
    ...
    async findOne(entity: unknown, where: Record<string, unknown>) {
      if (entity === ChannelArchitectProgramVersion)
        return where.id === version.id ? version : null;
      if (entity === ChannelArchitectProgramReview)
        return review && where.programVersionId === version.id ? review : null;
      ...
      if (entity === ChannelArchitectPilot) {
        return where.id === pilot.id && (!where.status || where.status === pilot.status)
          ? pilot
          : null;
      }
      if (entity === ChannelArchitectPilotCheckpoint) {
        return (
          checkpoints.find(
            (checkpoint) => checkpoint.id === where.id && checkpoint.pilotId === where.pilotId,
          ) ?? null
        );
      }
      return null;
    ...
    async nativeUpdate(
      ...
      const target =
        entity === ChannelArchitectProgram
          ? program
          : entity === ChannelArchitectPilot
            ? pilot
            : entity === ChannelArchitectPilotCheckpoint
              ? checkpoints.find((checkpoint) => checkpoint.id === where.id)
              : null;
      if (!target) return 0;
    ...
    async begin() {
      if (options.pilotStatusAtTransactionStart)
        pilot.status = options.pilotStatusAtTransactionStart;
    },
    async commit() {},
    async rollback() {},
    async flush() {},
...
describe("Open Mercato lifecycle commands", () => {
    ...
    await assert.rejects(
      async () =>
        reviewProgram.execute(
          ...
          commandContext(state, undefined, "reviewer-2", {
            reviews: [{ programVersionId: "version-1", decision: "approved" }],
          }),
        ),
  ...
  it("audits every lifecycle mutation without making immutable actions undoable or logging free text", async () => {
    ...
    assert.ok(
      commands.every(
        (command) => command.isUndoable === false && typeof command.buildLog === "function",
      ),
    );
    ...
    assert.deepEqual(reviewAudit?.payload, {
      __redoInput: { programVersionId: "version-1", reviewId: "review-1", decision: "approved" },
    });
  ...
  it("persists only the safe audit payload through the Open Mercato command bus", async () => {
    ...
    const commandPayload = auditEntries[0].commandPayload as {
      __redoInput: Record<string, unknown>;
    };
    ...
    await assert.rejects(
      async () =>
        createPilot.execute(
          ...
          pilotCommandContext({ reviewDecision: "approved", nextVersionAfterInitialRead: 2 })
            .runtime,
        ),
  ...
  it("requires all checkpoints to be resolved before completing an active pilot", async () => {
    ...
    await assert.rejects(
      async () =>
        updatePilotStatus.execute(
          {
            tenantId: "tenant-1",
            organizationId: "org-1",
            pilotId: "pilot-1",
            input: { status: "completed", outcome: "continue" },
          },
    ...
    const result = await updatePilotStatus.execute(
      {
        ...
        organizationId: "org-1",
        pilotId: "pilot-1",
        input: { status: "completed", outcome: "revise" },
      },
      ctx.runtime,
    );
      ...
      async () =>
        updatePilotCheckpoint.execute(
          ...
          pilotCommandContext({
            ...
            pilotStatusAtTransactionStart: "cancelled",
          }).runtime,
        ),
      /only be updated while a pilot is active or paused/,
    );
