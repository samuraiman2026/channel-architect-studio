...
const entryPoints = (await collect(join(root, "src"))).filter(
  (file) => /\.tsx?$/.test(file) && !file.split(/[\\/]/).includes("__integration__"),
);
await esbuild.build({
  entryPoints,
...
console.log(
  `Built ${entryPoints.length} Channel Architect module files from ${relative(process.cwd(), resolve(root, "src"))}.`,
);
