...
The current Studio is still browser-local. Its approval affordance is not authenticated approval. The module is not install-ready until generated migrations, module registration, authorization behavior, and the full package build are verified in an Open Mercato 0.6.x host. The host sandbox is not installed in this development environment, so those checks remain outstanding.
