...
The current Studio is still browser-local. It keeps drafts unapproved; a local endorsement note is not authenticated approval, and historical local approval labels are migrated to endorsement notes. In an isolated Open Mercato 0.6.x sandbox, generation discovered the package's five entities, program and pilot API routes, and backend page. Runtime loading resolved all five entity classes and registered all six commands. Open Mercato's schema generator produced the initial migration plus `.snapshot-open-mercato.json` against a disposable PostgreSQL 17 database; no migration was applied. The package build includes the migration, snapshot, and entity-discovery source needed by the standalone host. Studio production build and typecheck pass, as do 11 deterministic-engine, pilot-rule, and local-review migration acceptance tests. The full sandbox typecheck has unrelated generated-route and duplicate-Zod issues elsewhere in the host. OpenAPI's bundled approach cannot load `isolated-vm` under Node 26.7, but its static fallback generated the API document. End-to-end authorization/data checks remain outstanding, so the module is not yet install-ready.
